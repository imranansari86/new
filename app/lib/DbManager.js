...
/***********************Method for save data into DB for Guide List*************************************/
...
exports.Save_GuideAllList = function(guideData) {
	...
	try {
		...
		for (var i = 0; i < guideData.length; i++) {
			...
			if (guide.record_status == "a" || guide.record_status == "u") {//Add
				Ti.API.info('Check Update');
				var guideResultSet = db.execute('SELECT * FROM Guide WHERE guide_id=' + guide.guide_id);

				if (guideResultSet != null && guideResultSet.rowCount > 0) {
					db.execute('UPDATE Guide SET guide_type=?,location=?,heading=?,description=?,created_at=?,updated_at=?,status=?,delete_date=?,record_order=? WHERE guide_id=' + guide.guide_id, guide.guide_type, guide.location, guide.heading, guide.description, guide.created_at, guide.updated_at, guide.status, guide.delete_date, guide.record_order);
				} else {
					db.execute('INSERT INTO Guide(guide_id,guide_type,location,heading,description,created_at,updated_at,status,delete_date,record_order) VALUES (?,?,?,?,?,?,?,?,?,?)', guide.guide_id, guide.guide_type, guide.location, guide.heading, guide.description, guide.created_at, guide.updated_at, guide.status, guide.delete_date, guide.record_order);
				}
				
			} else if (guide.record_status == "d") {//Delete
...
/***********************Method to fetch data from DB for Guide Data*************************************/
...
exports.Get_guideAllList = function() {
	...
	try {
		...
		while (resultSet.isValidRow()) {
			...
			//Ti.API.info('guideArray : ' + JSON.stringify(guideArray));
			resultSet.next();
		}
	} catch(err) {
...
exports.Get_guideByIDForNotification = function(guidID) {
	...
	try {
		...
		//	Ti.API.info('resultset guide :' + JSON.stringify(resultSet));
		while (resultSet.isValidRow()) {
			var dataobj = {};
			dataobj.guide_id = resultSet.fieldByName('guide_id');
			dataobj.guide_type = resultSet.fieldByName('guide_type');
			...
			if (tips.record_status == "a" || tips.record_status == "u") {//Add
				...
				if (cats && cats.length > 0) {
					for (var j = 0; j < cats.length; j++) {
						...
						if (checkTipsCats != null && checkTipsCats.rowCount > 0) {
							continue;
						} else {
							var res = db.execute('INSERT INTO Tip_Category(tip_id,cat_id) VALUES (?,?)', tips.tip_id, cats[j]);
						}
					}
				}
			} else if (tips.record_status == "d") {//Delete
				db.execute('DELETE from Tips where tip_id=?', tips.tip_id);
			}
		}
	} catch(dberror) {
...
exports.Get_tipsAllListByCategory = function(category_id) {
	...
	try {
		db = Titanium.Database.open('VolunteerData');
		// resultSet = db.execute('SELECT * FROM Tips WHERE category_id=' + category_id + ' order by created_at desc');
...
exports.Get_tipsAllListByCategoryWithoutFeatured = function(category_id) {
	...
	try {
		db = Titanium.Database.open('VolunteerData');
		// resultSet = db.execute('SELECT * FROM Tips WHERE category_id=' + category_id + ' AND is_featured=0 order by created_at desc');
...
exports.Save_ContactsAllList = function(contactsData) {
	...
	try {
		...
		for (var i = 0; i < contactsData.length; i++) {
			...
			if (contacts.record_status == "a" || contacts.record_status == "u") {//Add
				Ti.API.info('Check Update');
				...
				if (contactsResultSet != null && contactsResultSet.rowCount > 0) {
					// alert("hello");
			...
			} else if (contacts.record_status == "d") {//Delete
				Ti.API.info('Check Delete');
		...
		while (resultSet.isValidRow()) {
			...
			try {
				if (keycontactLength > 0) {
					for (var i = 0; i < keycontactLength; i++) {
						if (resultSetkeyContacts.isValidRow()) {
							var keyContacts = {};
...
/***********************Method for save data into DB and get Data from DB for Category List*************************************/
	...
	try {
		...
		for (var i = 0; i < categoriesData.length; i++) {
			...
			if (categories.record_status == "a") {//Add
				...
				//alert(checkResult);
			} else if (categories.record_status == "u") {//Update
				Ti.API.info('Check Update');
				var categoriesResultSet = db.execute('SELECT * FROM Categories WHERE category_id=' + categories.category_id);

				if (categoriesResultSet != null && categoriesResultSet.rowCount > 0) {
					db.execute('UPDATE Categories SET name=?,description=?,parent_id=?,priority=?,image=?,status=?,delete_date=?,created_at=?,updated_at=? WHERE category_id=' + categories.category_id, categories.name, categories.description, categories.parent_id, categories.priority, categories.image, categories.status, categories.delete_date, categories.created_at, categories.updated_at);
				} else {
					db.execute('INSERT INTO Categories(category_id,name,description,parent_id,priority,image,status,delete_date,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)', categories.category_id, categories.name, categories.description, categories.parent_id, categories.priority, categories.image, categories.status, categories.delete_date, categories.created_at, categories.updated_at);
				}

			} else if (categories.record_status == "d") {//Delete
				Ti.API.info('Check Delete');
...
/***********************Method for save data into DB and get Data from DB for Location List*************************************/
...
exports.Save_LocationAllList = function(locationData) {
	...
	try {
		...
		for (var i = 0; i < locationData.length; i++) {
			...
			if (location.record_status == "a") {//Add
				Ti.API.info('Check Insert');
				...
				//alert(checkResult);
			} else if (location.record_status == "u") {//Update
				Ti.API.info('Check Update');
				var locationResultSet = db.execute('SELECT * FROM Location WHERE location_id=' + location.location_id);

				if (locationResultSet != null && locationResultSet.rowCount > 0) {
					db.execute('UPDATE Location SET location_name=?,status=?,created_at=?,updated_at=?,delete_date=? WHERE location_id=' + location.location_id, location.location_name, location.status, location.created_at, location.updated_at, location.delete_date);
				} else {
					db.execute('INSERT INTO Location(location_id,location_name,status,created_at,updated_at,delete_date) VALUES (?,?,?,?,?,?)', location.location_id, location.location_name, location.status, location.created_at, location.updated_at, location.delete_date);
				}

			} else if (location.record_status == "d") {//Delete
				Ti.API.info('Check Delete');
...
/***********************Method for save data into DB and get Data from DB for Users List*************************************/
...
/***********************Method for save data into DB and get Data from DB for Notification List*************************************/
...
exports.Save_NotificationAllList = function(notificationData) {
	...
	try {
		...
		for (var i = 0; i < notificationData.length; i++) {
			...
			if (notification.record_status == "a") {//Add
				Ti.API.info('Check Insert');
			...
			} else if (notification.record_status == "u") {//Update
				Ti.API.info('Check Update');
				var notificationResultSet = db.execute('SELECT * FROM Notification WHERE notification_id=' + notification.notification_id);

				if (notificationResultSet != null && notificationResultSet.rowCount > 0) {
					db.execute('UPDATE Notification SET location=?,location_type=?,notification_category=?,notification_type=?,notification_title=?,notification_description=?,notification_status=?,notification_link=?,link_ids=?,created_at=?,updated_at=?,delete_date=?,status=?,read_status=? WHERE notification_id=' + notification.notification_id, notification.location, notification.location_type, notification.notification_category, notification.notification_type, notification.notification_title, notification.notification_description, notification.notification_status, notification.notification_link, notification.link_ids, notification.created_at, notification.updated_at, notification.delete_date, notification.status, notification.read_status);
				} else {
					db.execute('INSERT INTO Notification(notification_id,location,location_type,notification_category,notification_type,notification_title,notification_description,notification_status,notification_link,link_ids,created_at,updated_at,delete_date,status,read_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', notification.notification_id, notification.location, notification.location_type, notification.notification_category, notification.notification_type, notification.notification_title, notification.notification_description, notification.notification_status, notification.notification_link, notification.link_ids, notification.created_at, notification.updated_at, notification.delete_date, notification.status, notification.read_status);
				}

			} else if (notification.record_status == "d") {//Delete
				Ti.API.info('Check Delete');
